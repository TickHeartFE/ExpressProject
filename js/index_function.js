...
// 对应的ip地址变量 当部署到真正的服务器的时候 ip的地址是不会更改的 将对应的ip进行绑定
var ip = "172.26.203.25:3000";
...
function sendRequest(idex, jdex) {
  // 跳转对应的页面
  var index = VideoIndex[idex][jdex];
...
// 封装好的ajax请求 用来更新对应视频地址
function sendAjax(url, searchIndex) {
  var xhr = new XMLHttpRequest();
  xhr.open("get", "http://" + ip + "/api/" + url + "?" + searchIndex);
  xhr.send();
  var params;
  xhr.onload = function () {
    // 然后对页面动态渲染
...
function sendAjax4(url) {
  ...
  xhr.onload = function () {
    ...
    for (var i = 0; i < len; i++) {
      ...
      // 存入对应的id变量
      VideoIndex[params[i]["VideoSet"]].push(params[i]["VideoId"]);
    }
    // 对应渲染对应的视频
...
function navClick(id) {
  ...
  index = webLocation.lastIndexOf("/");
  webLocation = webLocation.substring(index + 1, webLocation.length);
  console.log(webLocation);

  if (webLocation != "index.html" && webLocation != "video-page.html")
