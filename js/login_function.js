...
function checklogin() {
    ...
    if (userName.length == 0) {
        errorString += '帐户名称不能为空，请重新输入！<br/>';
        rgBadFields.userName = true;
    } else if (userName.length < 8 || userName.length > 20) {
        errorString += '帐户名称长度为 8-20个字符，请重新输入！<br/>';
        rgBadFields.userName = true;
    } else {
        var bNameOK = true;
        ...
        if (!bNameOK) {
            errorString += '帐户名称中只能包含a-z、A-Z、0-9 或 _ 字符，请重新输入！<br/>';
            rgBadFields.userName = true;
        }
    }
    ...
    if (passWord.length > 20) {
        errorString += '密码长度不能超过 20 个字符，请重新输入！<br/>';
        rgBadFields.passWord = true;

    }
    if (passWord.length == 0) {
        errorString += '密码长度不能为空，请重新输入！<br/>';
        rgBadFields.passWord = true;

    }

    if (passWord.length < 8 && passWord.length != 0) {
        errorString += '密码长度不能少于 8 个字符，请重新输入！<br/>';
        rgBadFields.passWord = true;
    }
